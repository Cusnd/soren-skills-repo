...
description: Prepare, adapt, launch, monitor, or debug AI training projects including LLM fine-tuning, RL/PPO, supervised PyTorch training, remote GPU runs, auto-updating TensorBoard real-time monitoring, checkpoints, metrics, logs, and SSH/tmux training workflows. Use when Codex works on training scripts, configs, observability, experiment records, or remote long-running AI jobs.
---

# AI Project Runner

Use this skill to make AI training projects observable and repeatable. Default to: Codex edits code, SSH/tmux runs long training on a Linux GPU server, TensorBoard monitors live training curves with automatic reload, and `logs/latest.log` captures terminal output.
...
2. Choose the narrow task:
   ...
   - **Instrument**: add TensorBoard logging and JSONL/config/checkpoint records to an existing training loop.
   - **Launch**: generate SSH/tmux/TensorBoard/tail commands for a remote run.
...
- **TensorBoard**: the dashboard for real-time training monitoring. Use it for high-frequency `loss`, `reward`, `lr`, image, histogram, embedding, graph, and step-level signals. Prefer short server reload intervals and frequent writer flushes when near-real-time charts matter.
- **Run records**: preserve reproducibility with config snapshots, `metrics/<run_name>.jsonl`, `runs/<run_name>`, `checkpoints/<run_name>`, `logs/<run_name>.log`, and `status.md` or `state.json`.
...
- `scripts/render_remote_training_commands.py`: generate SSH/tmux/TensorBoard/tail command templates without executing them.
- `scripts/check_remote_run.ps1`: read-only remote status helper using SSH from Windows PowerShell.

## Safety Rules

- Treat SSH credentials, API keys, W&B tokens, Hugging Face tokens, cloud credentials, and `.env` files as secrets.
- Never print full `.env` files, tokens, private keys, cookies, or password-like command arguments.
- Prefer local-only service binding on remote hosts: `--host 127.0.0.1` for TensorBoard, then use SSH tunnels.
