...
function Invoke-DidaCommand {
    ...
    if ($exitCode -ne 0) {
        ...
        throw "dida command failed: $Name (exit $exitCode)$detail"
    }
...
$projectColumnHostDeleted = $false
$groupDeleted = $false
$focusDeleted = $false
$failure = $null
...
try {
    ...
    $projectA = Invoke-DidaCommand -Name 'project create A' -Arguments @('project', 'create', '--name', "$prefix-project-a", '--kind', 'TASK', '--json') -Json
    ...
    $projectColumnHost = Invoke-DidaCommand -Name 'project create column host' -Arguments @('project', 'create', '--name', "$prefix-project-columns", '--view-mode', 'kanban', '--kind', 'TASK', '--json') -Json
    $projectColumnHostId = $projectColumnHost.id
    $created.projectColumnHostId = $projectColumnHostId
    Add-Step 'created project column host'

    $column = Invoke-DidaCommand -Name 'project column create' -Arguments @('project', 'column', 'create', $projectColumnHostId, '--name', "$prefix-column", '--json') -Json
    $columnId = $column.id
    $created.columnId = $columnId
    Add-Step 'created project column'

    Invoke-DidaCommand -Name 'project column update' -Arguments @('project', 'column', 'update', $projectColumnHostId, $columnId, '--name', "$prefix-column-updated", '--json') -Json -Attempts 3 | Out-Null
    ...
    Invoke-DidaCommand -Name 'task update' -Arguments @('task', 'update', $taskId, '--id', $taskId, '--project', $projectAId, '--title', "$prefix-task-updated", '--estimated-duration', '60', '--estimated-pomo', '1', '--json') -Json -Attempts 3 | Out-Null
    Add-Step 'updated task'

    $comment = Invoke-DidaCommand -Name 'task comment add' -Arguments @('task', 'comment', 'add', $projectAId, $taskId, '--title', "$prefix-comment", '--json') -Json -Attempts 3
    $commentId = $comment.id
    $created.commentId = $commentId
    Add-Step 'added task comment'

    $comments = @(Invoke-DidaCommand -Name 'task comment list' -Arguments @('task', 'comment', 'list', $projectAId, $taskId, '--json') -Json -Attempts 3)
    ...
    Invoke-DidaCommand -Name 'task move' -Arguments @('task', 'move', '--from', $projectAId, '--to', $projectBId, '--task', $taskId, '--json') -Json -Attempts 3 | Out-Null
    $taskProjectId = $projectBId
    Add-Step 'moved task'

    Invoke-DidaCommand -Name 'task get after move' -Arguments @('task', 'get', $projectBId, $taskId, '--json') -Json -Attempts 3 | Out-Null
    ...
    if ($projectColumnHostId -and -not $projectColumnHostDeleted) {
        Remove-IfNeeded -Type 'project' -Id $projectColumnHostId -Arguments @('project', 'delete', $projectColumnHostId)
        $projectColumnHostDeleted = $true
    }
